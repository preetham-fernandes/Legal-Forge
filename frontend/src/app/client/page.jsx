...
import LegalChatbot from "@/components/legal_chatbot";
import ContractReview from "@/components/contract_review";
import DocSummary from "@/components/doc_summary";
...
const features = [
  ...
  { name: "Document Summary", icon: FileTextIcon },
];
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white">
      ...
      <div className="flex flex-1 overflow-hidden">
        <aside
          className={`bg-[#0A2E4D] text-white w-64 flex-shrink-0 overflow-y-auto transition-all duration-300 ease-in-out border-r border-[#DBA865]`}
        >
          <nav className="p-4 space-y-2">
            {features.map((feature) => (
              <Button
                key={feature.name}
                variant="ghost"
                className={`w-full justify-start ${selectedFeature === feature.name ? "bg-[#DBA865] text-[#0A2E4D]" : "text-[#DBA865] hover:bg-[#DBA865] hover:text-[#0A2E4D]"}`}
